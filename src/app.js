...
import { addDiscovery, readProgress, writeProgress } from './storage.js?v=090';
import { escapeHtml, renderMarkdown } from './markdown.js?v=090';
import { downloadPostcard } from './postcard.js?v=090';
import { artImage } from './art-paths.js?v=090';
import { iconSvg } from './illustrations.js?v=090';
...
async function bootWorld(){
  ...
  try{
    const module=await Promise.race([import('./world.js?v=090'),new Promise((_,reject)=>{timeout=setTimeout(()=>reject(new Error('3D 依赖下载超时，文章仍可阅读。')),15000);})]);
